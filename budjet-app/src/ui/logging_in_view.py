...
class LoginView:
    ...
    def _view(self):
        ...
        self._error_label = ttk.Label(master=self._frame,textvariable=self._error_var,foreground='red')
        label = ttk.Label(master=self._frame, text="Log in")
        username_label = ttk.Label(master=self._frame, text="Username")
        self._username_entry = ttk.Entry(master=self._frame)
        password_label = ttk.Label(master=self._frame, text="Password")
        self._password_entry = ttk.Entry(master=self._frame)
        login_button = ttk.Button(master=self._frame, text="Log in", command=self._loggin_in_handler) 
        not_user_label = ttk.Label(master=self._frame, text="Don't have a user?")
        create_create_button = ttk.Button(master=self._frame, text="Create a new user", command=self._handle_creating_user) 
        ...
        login_button.grid(row=6,columnspan=2, sticky=(constants.E, constants.W), padx=80, pady=5)
        not_user_label.grid(row=7, column=0,sticky=(constants.E, constants.W), padx=5, pady=5)
        create_create_button.grid(row=7,column=0, sticky=(constants.W), padx=140, pady=5)
