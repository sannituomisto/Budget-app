...
class BudgetRepository:
    ...
    def expense_sum_by_category(self, username, category):
        ...
        cursor = self._connection.cursor()
        cursor.execute(
            "SELECT COALESCE(SUM(amount),0) FROM Expense WHERE username= ? AND category= ?",
            [username, category])
        row = cursor.fetchone()[0]
        return row
